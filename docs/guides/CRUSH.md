...
CannaAI (CultivAI Pro) is an advanced cannabis cultivation management system built with Next.js 15, TypeScript, and modern web technologies. It features AI-powered plant health analysis, real-time sensor monitoring, automation controls, and comprehensive analytics for cannabis cultivation.
...
- `npm run start:win` - Windows-specific production start command
- `npm run lint` - Run ESLint for code quality checks
- `startup.bat` - Windows startup script with interactive options
...
- **Language**: TypeScript 5 with relaxed strictness (`noImplicitAny: false`)
...
- AI provider settings UI in `src/components/ai/AIProviderSettings.tsx`
...
- ESLint rules heavily relaxed for rapid development
...
- Windows-specific commands available via `startup.bat`
...
- Database auto-creation via `startup.bat`
...
- Most rules disabled for rapid development
- TypeScript rules relaxed
- React rules lenient
- Next.js rules mostly ignored
...
### Windows Development
- `startup.bat` provides interactive development environment
...
- Use `startup.bat` on Windows for simplified development
- Port 3000 is default; script handles conflicts automatically
- Database is auto-created on first run
- All TypeScript/ESLint errors are ignored in builds

### AI Integration
- Z-AI SDK requires API configuration
- LM Studio needs local server running
- OpenRouter requires API key
- Model selection affects analysis quality

### Real-time Features
- Socket.IO requires WebSocket-compatible connections
- Mock sensor data simulates real cultivation environment
- Automation controls are simulated (not connected to real hardware)

### Database Limitations
- SQLite for development (can be swapped for PostgreSQL in production)
- Current schema is minimal (User/Post only)
- Extensible for cultivation-specific data models
