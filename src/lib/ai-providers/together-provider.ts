 ...
 * Open-source model access with competitive pricing
 */
...
export class TogetherProvider extends BaseProvider {
  constructor(config: any) {
    super({
      ...
      pricing: {
        input: 0.06, // Competitive pricing for open-source models
        output: 0.18,
        currency: 'USD',
        visionCostPerImage: 0.003,
        ...config.pricing
      }
    });
  }
  ...
  protected normalizeRequest(request: AIRequest): any {
    ...
    return {
      ...
      top_k: request.topK ?? this.config.topK ?? 50,
  ...
  protected normalizeResponse(response: any, metadata: any): AIResponse {
    ...
    return {
      id: response.id || `together_${Date.now()}`,
