...
function performAutoAnalysis(sensorData: any, strain?: string, growthStage?: string) {
  ...
  if (lightIntensity) {
    if (lightIntensity < ANALYSIS_THRESHOLDS.lightIntensity.critical.min || lightIntensity > ANALYSIS_THRESHOLDS.lightIntensity.critical.max) {
      healthScore -= 15;
      issues.push(`Critical light intensity: ${lightIntensity} μmol`);
      ...
      healthScore -= 8;
      issues.push(`Suboptimal light intensity: ${lightIntensity} μmol`);
      if (diagnosis === 'Optimal Conditions') diagnosis = 'Light Imbalance';
    } else {
      strengths.push(`Optimal light intensity: ${lightIntensity} μmol`);
    }
  }
...
function generateRecommendations(analysis: any, sensorData: any, growthStage?: string) {
  ...
  if (tempF) {
    if (tempF < 65) {
      recommendations.push(`🌡️ Increase temperature to 68-78°F. Consider heating or better insulation.`);
    } else if (tempF > 82) {
      recommendations.push(`🌡️ Reduce temperature to 68-78°F. Improve ventilation or add cooling.`);
    }
  }
  ...
  if (sensorData.humidity) {
    if (sensorData.humidity < 30) {
      recommendations.push(`💧 Increase humidity to 40-60%. Add humidifiers or mist plants.`);
    } else if (sensorData.humidity > 70) {
      recommendations.push(`💧 Reduce humidity to 40-60%. Improve air circulation and ventilation.`);
    }
  }
  ...
  if (sensorData.ph) {
    if (sensorData.ph < 5.8) {
      recommendations.push(`⚗️ Raise pH to 6.0-7.0. Add pH up solution and flush if needed.`);
    } else if (sensorData.ph > 7.2) {
      recommendations.push(`⚗️ Lower pH to 6.0-7.0. Add pH down solution and flush if needed.`);
    }
  }
  ...
  if (sensorData.soilMoisture) {
    if (sensorData.soilMoisture < 30) {
      recommendations.push(`💦 Water plants soon. Soil moisture is critically low.`);
    } else if (sensorData.soilMoisture > 80) {
      recommendations.push(`💦 Stop watering. Risk of root rot due to overwatering.`);
    }
  }
  ...
  if (sensorData.lightIntensity) {
    if (sensorData.lightIntensity < 300) {
      recommendations.push(`💡 Increase light intensity to 400-800 μmol. Move lights closer or add more.`);
    } else if (sensorData.lightIntensity > 1000) {
      recommendations.push(`💡 Reduce light intensity to prevent light burn. Move lights further away.`);
    }
  }
  ...
  if (sensorData.ec) {
    if (sensorData.ec < 1.0) {
      recommendations.push(`🧪 Increase nutrient concentration. EC is too low for healthy growth.`);
    } else if (sensorData.ec > 2.8) {
      recommendations.push(`🧪 Decrease nutrient concentration. Risk of nutrient burn.`);
    }
  }
  ...
  if (growthStage) {
    switch (growthStage.toLowerCase()) {
      case 'seedling':
        recommendations.push('🌱 Maintain high humidity (60-70%) and moderate light for seedlings.');
        break;
      case 'vegetative':
        recommendations.push('🌿 Focus on nitrogen-rich nutrients and 18+ hours of light during vegetative stage.');
        break;
      case 'flowering':
        recommendations.push('🌸 Switch to bloom nutrients and 12/12 light schedule for flowering.');
        break;
    }
  }
  ...
  if (issues.length === 0) {
    recommendations.push('✅ All parameters look good! Continue monitoring and maintain current conditions.');
  }

  recommendations.push('📊 Continue regular monitoring of all environmental parameters.');
  recommendations.push('🔧 Check equipment regularly to ensure consistent performance.');
