...
describe('AgentCommandProvider Hermes proxy resilience', () => {
  ...
  test('preserves native image parts for Hermes and OpenClaw', async () => {
    const execute = jest.spyOn(AgentCommandProvider.prototype, 'execute').mockResolvedValue({
      id: 'agent-response',
      object: 'chat.completion',
      created: 0,
    ...
    const messages = [{
      ...
      content: [
        { type: 'text', text: 'What is shown?' },
        { type: 'image', image_url: 'data:image/heic;base64,ZmFrZQ==' },
      ],
    }];

    await expect(executeWithHermes(messages)).resolves.toEqual(expect.objectContaining({ success: true }));
    expect(execute.mock.calls[0][0].messages[0].image).toBe('data:image/heic;base64,ZmFrZQ==');
    await expect(executeWithOpenClaw(messages)).resolves.toEqual(expect.objectContaining({ success: true }));
    expect(execute.mock.calls[1][0].messages[0].image).toBe('data:image/heic;base64,ZmFrZQ==');
  });
});
